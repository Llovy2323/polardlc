...
import snill.client.api.utils.render.font.ReplaceSymbols;
...
public class EntityESP extends Module {
    ...
    private void drawTag(EventRender.Default event, PlayerEntity player, ScreenRect rect, Font font) {
        ...
        String rightBracket = "";
        boolean isFriend = Snill.INSTANCE.friendStorage != null
                && Snill.INSTANCE.friendStorage.isFriend(player.getName().getString());
    ...
    private boolean shouldProcessPlayer(PlayerEntity player, boolean skipInvisible) {
        ...
        boolean hasVisibleEquipment = hasVisibleEquipment(player);
        if (AntiBot.checkBot(player) && !hasVisibleEquipment) {
            return false;
        }
        if (player == mc.player && isInFirstPerson()) {
            return false;
        }
        if (skipInvisible && isInvisibleToEsp(player) && !hasVisibleEquipment && !canRenderInvisiblePlayer(player)) {
            return false;
        }
        return true;
    }

    private boolean isInvisibleToEsp(PlayerEntity player) {
        return player.isInvisible()
                || player.hasStatusEffect(net.minecraft.entity.effect.StatusEffects.INVISIBILITY)
                || (mc.player != null && player.isInvisibleTo(mc.player));
    }

    private boolean hasVisibleEquipment(PlayerEntity player) {
        if (player == null) {
            return false;
        }
    ...
    private void render3DBox(MatrixStack matrices, Entity entity, float tickDelta) {
        ...
        boolean isFriend = entity instanceof PlayerEntity player
                && Snill.INSTANCE.friendStorage != null
                && Snill.INSTANCE.friendStorage.isFriend(player.getName().getString());
        int boxColor;
        if (isFriend) {
