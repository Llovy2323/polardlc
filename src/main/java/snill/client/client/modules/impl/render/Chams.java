...
import snill.client.mixin.LivingEntityRendererAccessor;
...
public class Chams extends Module {
    ...
    private void setupModelMatrix(MatrixStack matrices, PlayerEntityRenderState state, PlayerEntityRenderer renderer, Vec3d cameraPos, PlayerEntity player, float tickDelta) {
        ...
        accessor.snill$setupTransforms(state, matrices, state.bodyYaw, baseScale);
        matrices.scale(-1.0f, -1.0f, 1.0f);
        accessor.snill$scale(state, matrices);
    ...
    public boolean affects(PlayerEntity player) {
        ...
        if (player == mc.player) {
            return rendering.is(TARGET_SELF)
                    && mc.options.getPerspective() != Perspective.FIRST_PERSON
                    && !isSelfPerspectiveSwitching();
        }
        if (isFriend(player)) {
    ...
    private boolean isSelfPerspectiveSwitching() {
        InterpolateF5 interpolateF5 = ModuleClass.INSTANCE != null ? ModuleClass.INSTANCE.interpolateF5 : null;
        return interpolateF5 != null && interpolateF5.isEnable() && interpolateF5.isSwitchAnimating();
    }
