...
public class BlockOverlay extends Module {
    ...
    private static Box predictionBox;
    private final ModeSetting mode = new ModeSetting("Режим", "Шейдер", "Шейдер", "Нитки");
    private final ModeSetting shaderPreset = new ModeSetting("Пресет шейдера", "Cloud",
            "Cloud", "Balatro", "Plasma")
    ...
    public BlockOverlay() {
        super("BlockOverlay", "Обводка блока", ModuleCategory.RENDER);
        addSettings(mode, shaderPreset, waveSpeed, waveScale, lineSpeed, lineJitter, outline, glow, fill, alpha, smooth, staticOutline);
    }

    /**
     * Lets combat prediction reuse the exact BlockOverlay shader for its
     * interception point. A null value returns the overlay to the aimed block.
     */
    public static void setPredictionBox(Box box) {
        predictionBox = box;
    }
    ...
    public void onRender3D(Event3DRender event) {
        ...
        Box worldBox = predictionBox != null ? predictionBox : getTargetedBlockBox();
        if (worldBox != null) {
            ...
            overlayAlpha = lerpValue(overlayAlpha, 1.0f, Math.min(1.0f, smooth.get() * 1.35f));
        } else {
    ...
    public void onRender2D(EventRender.Default event) {
        if (!hasMask || maskBuffer == null || overlayAlpha <= 0.02f) return;
        if (mode.is("Нитки")) return;

        ShaderProgram shader = mc.getShaderLoader().getOrCreateProgram(getBlockOverlayShaderKey());
        ...
        RenderSystem.setShader(getBlockOverlayShaderKey());
        ...
        setUniform(shader, "alpha", (lineMode ? 1.0f : alpha.get()) * overlayAlpha);
    ...
    private ShaderProgramKey getBlockOverlayShaderKey() {
        return switch (shaderPreset.getCurrent()) {
    ...
    private Box getTargetedBlockBox() {
        HitResult hit = getOutlineHitResult();
    ...
    private HitResult getOutlineHitResult() {
        if (mc.player == null || mc.world == null) return mc.crosshairTarget;
        ...
        return outlineHit.getType() == HitResult.Type.BLOCK ? outlineHit : mc.crosshairTarget;
    }
    ...
    private void drawAnimatedWeb(Matrix4f matrix, Box box) {
        ...
        int baseAlpha = Math.max(0, Math.min(255, (int) (alpha.get() * overlayAlpha * 210.0f)));
        ...
        RenderSystem.depthMask(false);
        RenderSystem.setShader(ShaderProgramKeys.POSITION_COLOR);
        drawFilledBox(matrix, box, ColorUtils.setAlphaColor(themeColor, (int) (alpha.get() * overlayAlpha * fill.get() * 170.0f)));
    ...
    private void updateCachedThemeColors() {
        if (Snill.INSTANCE == null || Snill.INSTANCE.themeStorage == null || Snill.INSTANCE.themeStorage.getThemes() == null) {
