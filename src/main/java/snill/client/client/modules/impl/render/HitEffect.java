...
public class HitEffect extends Module {
    ...
    public static HitEffect INSTANCE = new HitEffect();

    private final ModeSetting mode = new ModeSetting("Режим", "Angel", "Angel", "Ghost");
    ...
    private static final float GLOW_INTENSITY = 0.18f;
    private static final float HALO_SIZE = 0.46f;
    private final List<TotemGhost> ghosts = new CopyOnWriteArrayList<>();

    public HitEffect() {
        super("HitEffect", "Отображает эффект при ударе по игроку", ModuleCategory.RENDER);
        addSettings(mode, visuals.visible(() -> false), riseHeight, duration);
    }
    ...
    private Identifier getSkinTexture() {
        return Identifier.of("snill", "textures/skin/skin.png");
    }
    @EventLink
    ...
    private void renderGhosts(MatrixStack matrices, float tickDelta) {
        ...
        for (TotemGhost ghost : ghosts) {
            ...
            float alpha = (1.0f - easeInCubic(progress)) * 1.0f;
