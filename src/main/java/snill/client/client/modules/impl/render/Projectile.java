...
public class Projectile extends Module {
    ...
    private record ImpactPoint(Vec3d pos, float seconds, ItemStack icon) {
    }

    private static final Identifier BLOOM_TEXTURE = Identifier.of("snill", "textures/particle/bloom.png");
    ...
    public void onRender3D(Event3DRender event) {
        ...
        for (EnderPearlEntity pearl : mc.world.getEntitiesByClass(EnderPearlEntity.class, searchBox, Entity::isAlive)) {
            ...
            impactPoints.add(new ImpactPoint(impactPos, seconds, Items.ENDER_PEARL.getDefaultStack()));
    ...
    public void onRender2D(EventRender.Default event) {
        ...
        for (ImpactPoint impact : impactPoints) {
            ...
            float boxWidth = textWidth + 28.0f;
            float boxHeight = 18.0f;
            float x = (float) screen.x - (boxWidth / 2.0f);
            float y = (float) screen.y - 9;

            RenderUtils.drawDefaultHudThemedPanel(matrices, x, y, boxWidth, boxHeight, 3, 3.5f, themeColor);
            matrices.push();
            matrices.translate(x + 5.0f, y + 1.0f, 120.0f);
            matrices.scale(0.85f, 0.85f, 1.0f);
            event.getContext().drawItem(impact.icon(), 0, 0);
            matrices.pop();
            font.drawString(matrices, text, x + 22.0f, y + 7.1f, 0xFFFFFFFF);
        }
    }
