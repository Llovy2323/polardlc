...
public class ClickGuiState {
    ...
    public void refreshModules() {
        ...
        allModules.addAll(ModuleClass.INSTANCE.getObject().stream()
                .filter(module -> !"AutoForest".equals(module.getName()))
                .toList());
        for (Module.ModuleCategory category : Module.ModuleCategory.values()) {
            ...
            categoryScrollAnimation.putIfAbsent(category, new AnimationUtils(0f, 4.5f, Easings.SINE_IN_OUT));
        }
    }

    public void updatePosition(float screenW, float screenH, int categoryCount) {
        float totalCategoriesWidth = ClickGuiLayout.getTotalCategoriesWidth(categoryCount);
        this.x = (screenW / 2F) - (totalCategoriesWidth / 2F);
        this.y = (screenH / 2F) - (ClickGuiLayout.HEIGHT / 2F);
    }

    public void updatePosition(Window window, int categoryCount, float guiScale) {
        if (window == null) return;
        float scale = Math.max(0.01f, guiScale);
        updatePosition(window.getScaledWidth() / scale, window.getScaledHeight() / scale, categoryCount);
    }

    public void updatePosition(Window window, int categoryCount) {
        updatePosition(window, categoryCount, 1.0f);
    }
    ...
    public float getScroll(Module.ModuleCategory category) {
        AnimationUtils animation = categoryScrollAnimation.computeIfAbsent(category, key -> new AnimationUtils(0f, 4.5f, Easings.SINE_IN_OUT));
    ...
    public float getOpenProgress(Module module) {
        AnimationUtils animation = moduleOpenAnimation.computeIfAbsent(
                module,
                key -> new AnimationUtils(module.isOpen() ? 1f : 0f, 6.0f, Easings.SINE_IN_OUT)
        );
    ...
    public AnimationUtils getBooleanBackgroundAnimation(BooleanSetting setting) {
        return booleanBackgroundAnimation.computeIfAbsent(
                setting,
                key -> new AnimationUtils(setting.isState() ? 1f : 0f, 6.5f, Easings.SINE_IN_OUT)
        );
    }
    ...
    public AnimationUtils getBooleanCircleAnimation(BooleanSetting setting) {
        return booleanCircleAnimation.computeIfAbsent(
                setting,
                key -> new AnimationUtils(setting.isState() ? 1f : 0f, 5.0f, Easings.SINE_IN_OUT)
        );
    }
    ...
    public AnimationUtils getSliderAnimation(FloatSetting setting) {
        return sliderAnimation.computeIfAbsent(
                setting,
                key -> new AnimationUtils(getSliderPos(setting), 6.8f, Easings.SINE_IN_OUT)
        );
    }
    ...
    public AnimationUtils getModeAnimation(String key, boolean selected) {
        return modeAnimation.computeIfAbsent(
                key,
                unused -> new AnimationUtils(selected ? 1f : 0f, 5.5f, Easings.SINE_IN_OUT)
        );
    }
    ...
    public AnimationUtils getBindAnimation(String key, boolean active) {
        return bindAnimation.computeIfAbsent(
                key,
                unused -> new AnimationUtils(active ? 1f : 0f, 5.5f, Easings.SINE_IN_OUT)
        );
    }
    ...
    public AnimationUtils getTextHoverAnimation(String key, boolean hovered) {
        return textHoverAnimation.computeIfAbsent(
                key,
                unused -> new AnimationUtils(hovered ? 1f : 0f, 5.0f, Easings.SINE_IN_OUT)
        );
    }
