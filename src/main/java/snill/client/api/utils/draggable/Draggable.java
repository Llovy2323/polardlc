package snill.client.api.utils.draggable;
...
public class Draggable implements QClient {
    ...
    public final boolean onClick(double mouseX, double mouseY, int button) {
        if (button == 0 && HoveringUtils.isInRegion(mouseX, mouseY, this.xPos, this.yPos, this.width, this.height)) {
            ...
            this.startX = (float) (mouseX - (double) this.xPos);
            this.startY = (float) (mouseY - (double) this.yPos);
            this.targetXPos = this.xPos;
            this.targetYPos = this.yPos;
            this.lastDragMouseX = (float) mouseX;
