package snill.client.api.utils.render;
...
import snill.client.api.utils.render.glow.GlowCallback;
import snill.client.api.utils.render.glow.GlowProgram;
...
public class RenderUtils implements QClient {
    ...
    private static int liquidBlurLastWidth = -1;
    private static int liquidBlurLastHeight = -1;
    private static float liquidBlurOffset = 1.0f;
    ...
    public static void beginLiquidBlurFrame() {
        ...
        if (liquidBlurBuffer1 == null || liquidBlurBuffer2 == null
                || liquidBlurLastWidth != fbWidth || liquidBlurLastHeight != fbHeight) {
            deleteLiquidBlurBuffers();
            liquidBlurBuffer1 = createLiquidBlurBuffer(fbWidth, fbHeight);
            liquidBlurBuffer2 = createLiquidBlurBuffer(fbWidth, fbHeight);
        ...
        blitLiquidBlurPass(kawaseDown, mc.getFramebuffer(), liquidBlurBuffer1, fbWidth, fbHeight, scaledW, scaledH);

        Framebuffer[] buffers = {liquidBlurBuffer1, liquidBlurBuffer2};
        for (int i = 1; i < LIQUID_BLUR_ITERATIONS; i++) {
            Framebuffer src = buffers[(i + 1) % 2];
            Framebuffer dst = buffers[i % 2];
            blitLiquidBlurPass(kawaseDown, src, dst, src.textureWidth, src.textureHeight, scaledW, scaledH);
        }

        for (int i = 0; i < LIQUID_BLUR_ITERATIONS; i++) {
            Framebuffer src = buffers[i % 2];
            Framebuffer dst = buffers[(i + 1) % 2];
            blitLiquidBlurPass(kawaseUp, src, dst, src.textureWidth, src.textureHeight, scaledW, scaledH);
        }

        liquidBlurResultBuffer = buffers[LIQUID_BLUR_ITERATIONS % 2];
    ...
    private static void blitLiquidBlurPass(ShaderProgram shader, Framebuffer src, Framebuffer dst,
                                           int texW, int texH, float quadW, float quadH) {
        ...
        setKawaseUniforms(shader, texW, texH);
        drawFullscreenQuad(quadW, quadH);
    ...
    private static void setKawaseUniforms(ShaderProgram shader, int texW, int texH) {
        ...
        if (res != null) res.set(1.0f / texW, 1.0f / texH);
        if (off != null) off.set(liquidBlurOffset);
        if (sat != null) sat.set(1.0f);
        if (tintI != null) tintI.set(0.0f);
        if (tintC != null) tintC.set(1.0f, 1.0f, 1.0f);
    }

    private static void drawFullscreenQuad(float w, float h) {
        BufferBuilder bb = Tessellator.getInstance()
                .begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
        bb.vertex(0, 0, 0).texture(0, 1).color(1f, 1f, 1f, 1f);
        bb.vertex(0, h, 0).texture(0, 0).color(1f, 1f, 1f, 1f);
        bb.vertex(w, h, 0).texture(1, 0).color(1f, 1f, 1f, 1f);
        bb.vertex(w, 0, 0).texture(1, 1).color(1f, 1f, 1f, 1f);
        BufferRenderer.drawWithGlobalProgram(bb.end());
    }

    private static Framebuffer createLiquidBlurBuffer(int w, int h) {
    ...
    public void drawRoundedImage(MatrixStack matrices, Identifier texture, float x, float y, float size, float radius, float alpha) {
        if (texture == null) {
            return;
        }
        ...
        if (uUniform != null) uUniform.set(0.0f);
        if (vUniform != null) vUniform.set(0.0f);
        if (wUniform != null) wUniform.set(1.0f);
        if (hUniform != null) hUniform.set(1.0f);
        if (hurtPercentUniform != null) hurtPercentUniform.set(0.0f);
    ...
    public void drawDefaultHudElementRects(MatrixStack matrices, float x, float y, float width, float height, int themeColor, boolean drawPattern) {
        drawBlur(matrices, x, y, width, height, 4, 5, ColorUtils.rgba(255,255,255,255));
        drawBlur(matrices, x, y, width, height, 4, 5, ColorUtils.rgba(0,0,0,180));
        drawRoundedRect(matrices, x, y, width, height, 4, ColorUtils.rgba(20, 20, 20, 100));
        if (drawPattern) {
    ...
    public void drawDefaultHudPanel(MatrixStack matrices, float x, float y, float width, float height, float gradientRadius, float borderRadius, int borderColor, int topColor, int bottomColor) {
        drawDefaultHudPanelGlass(matrices, x, y, width, height, borderRadius);
    }
    public void drawDefaultHudPanel1(MatrixStack matrices, float x, float y, float width, float height, float gradientRadius, float borderRadius, int borderColor, int topColor, int bottomColor) {
        drawDefaultHudPanelGlass(matrices, x, y, width, height, borderRadius);
    }
    ...
    private void drawDefaultHudPanelGlass(MatrixStack matrices, float x, float y, float width, float height, float borderRadius) {
        drawWatermarkGlass(matrices, x, y, width, height, borderRadius);
    }

    private void drawWatermarkGlass(MatrixStack matrices, float x, float y, float width, float height, float radius) {
        drawBlur(matrices, x, y, width, height, radius, 5f, ColorUtils.rgba(255, 255, 255, 255));
        drawBlur(matrices, x, y, width, height, radius, 5f, ColorUtils.rgba(0, 0, 0, 180));
        drawRoundedRect(matrices, x, y, width, height, radius, ColorUtils.rgba(20, 20, 20, 100));
    }
    ...
    public void drawRoundedRectOutline(MatrixStack matrices, float x, float y, float width, float height,
                                       float radius, float outline, int outlineColor) {
    ...
    public void drawBlur(MatrixStack matrices, float x, float y, float width, float height,
        ...
        RenderSystem.setShaderTexture(0, getLiquidBlurTexture());
        RenderSystem.setShader(ShaderUtils.roundedTexture);

        int screenWidth = mc.getWindow().getScaledWidth();
        int screenHeight = mc.getWindow().getScaledHeight();

        float screenX = matrix.m00() * x + matrix.m30();
        float screenY = matrix.m11() * y + matrix.m31();
        float screenW = width * matrix.m00();
        float screenH = height * matrix.m11();

        float u1 = screenX / screenWidth;
        float v1 = (screenHeight - screenY) / screenHeight;
        float u2 = (screenX + screenW) / screenWidth;
        float v2 = (screenHeight - screenY - screenH) / screenHeight;
    ...
    public void drawBlur(MatrixStack matrices, float x, float y, float width, float height,
        ...
        setLiquidBlurOffset(blurStrength);
        drawBlur(matrices, x, y, width, height, topLeft, topRight, bottomRight, bottomLeft, color);
    }
    ...
    public void drawBlurRect(MatrixStack matrices, float x, float y, float width, float height, float radius, int color, float blurStrength) {
        drawBlur(matrices, x, y, width, height, radius, blurStrength, color);
    }

    public void drawBlurRect(MatrixStack matrices, float x, float y, float width, float height, float topLeft, float topRight, float bottomRight, float bottomLeft, int color, float blurStrength) {
        drawBlur(matrices, x, y, width, height, topLeft, topRight, bottomRight, bottomLeft, blurStrength, color);
    }

    public void drawBlurRect(MatrixStack matrices, float x, float y, float width, float height, float radius, int color, int color2, float blurStrength) {
        drawBlur(matrices, x, y, width, height, radius, blurStrength, color);
    }

    public void drawBlurRect(MatrixStack matrices, float x, float y, float width, float height, float radius, int color, int color2, int color3, int color4, float blurStrength) {
        drawBlur(matrices, x, y, width, height, radius, blurStrength, color);
    }

    public void drawLiquidGlass(MatrixStack matrices, float x, float y, float width, float height,
                                float topLeft, float topRight, float bottomRight, float bottomLeft, int color, float globalAlpha, float fresnelPower, int fresnelColor,
                                float baseAlpha, boolean fresnelInvert, float fresnelMix, float distortStrength, float squirt, boolean clean) {
        int textureId;
        if (clean) {
            textureId = mc.getFramebuffer().getColorAttachment();
        } else {
            requestLiquidBlur();
            textureId = getLiquidBlurTexture();
            if (textureId == 0) {
                textureId = mc.getFramebuffer().getColorAttachment();
            }
        }
    ...
    public void drawLiquidGlass(MatrixStack matrices, float x, float y, float width, float height,
                                float radius, float squirt, int color) {
        float opacity = 0.8f;
        float strength = height >= 240.0f ? 2.0f : 1.0f;
        float distortion = 0.08f;
        float rounding = radius;

        drawLiquidGlass(matrices, x, y, width, height, rounding, rounding, rounding, rounding, color,
                opacity, strength, ColorUtils.rgba(255, 255, 255, 255),
                0.2f, false, 1.0f, distortion, squirt, false);
    }

    public void drawLiquidGlass(MatrixStack matrices, float x, float y, float width, float height,
                                float radius, float squirt, int color, boolean clean) {
        float opacity = 0.8f;
        float strength = height >= 240.0f ? 2.0f : 1.0f;
        float distortion = 0.08f;
        float rounding = radius;
        ...
        drawLiquidGlass(matrices, x, y, width, height, rounding, rounding, rounding, rounding, color,
                opacity, strength, ColorUtils.rgba(255, 255, 255, 255),
                0.2f, false, 1.0f, distortion, squirt, clean);
    }

    public void drawLiquidGlass(MatrixStack matrices, float x, float y, float width, float height, int color) {
        drawLiquidGlass(matrices, x, y, width, height, 0.0f, 1.0f, color);
    }

    public void drawLiquidGlass(MatrixStack matrices, float x, float y, float width, float height, int color, boolean clean) {
        drawLiquidGlass(matrices, x, y, width, height, 0.0f, 1.0f, color, clean);
    }

    public void drawLiquidBlur(MatrixStack matrices, float x, float y, float width, float height,
                               float topLeft, float topRight, float bottomRight, float bottomLeft,
                               int color, float globalAlpha, float fresnelPower, int fresnelColor,
                               float baseAlpha, boolean fresnelInvert, float fresnelMix, float distortStrength, float squirt, boolean clean) {
        drawLiquidGlass(matrices, x, y, width, height, topLeft, topRight, bottomRight, bottomLeft, color,
                globalAlpha, fresnelPower, fresnelColor, baseAlpha, fresnelInvert, fresnelMix, distortStrength, squirt, clean);
    }

    public void drawLiquidBlur(MatrixStack matrices, float x, float y, float width, float height,
                               float radius, int color, float globalAlpha, float fresnelPower, int fresnelColor,
                               float baseAlpha, boolean fresnelInvert, float fresnelMix, float distortStrength, float squirt, boolean clean) {
        drawLiquidBlur(matrices, x, y, width, height, radius, radius, radius, radius, color,
                globalAlpha, fresnelPower, fresnelColor, baseAlpha, fresnelInvert, fresnelMix, distortStrength, squirt, clean);
    }
}
