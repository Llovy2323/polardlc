package snill.client.api.utils.combat;
...
public class PredictUtils implements QClient {
    ...
    public static Vec3d bypasselytrahacking(LivingEntity target, float forwardTicks) {
        Vec3d look = Vec3d.fromPolar(target.getPitch(), target.getYaw());
        Vec3d rotation = target.getRotationVector();
        Vec3d deltaToPlayer = target.getPos().add(0.0, target.getHeight() * 0.6f, 0.0).subtract(mc.player.getEyePos());
        Vec3d blendedDirection = look.normalize().lerp(rotation, look.length());
        return deltaToPlayer.add(blendedDirection.normalize().multiply(forwardTicks));
    }

    public static Vec3d bypasselytrahacking(LivingEntity target) {
        ElytraTarget elytraTarget = ElytraTarget.INSTANCE;
        float forwardTicks = elytraTarget != null ? elytraTarget.getForwardTicks() : 0.0f;
        return bypasselytrahacking(target, forwardTicks);
    }
